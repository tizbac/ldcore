...
struct TRINITY_DLL_DECL npc_target_dummyAI : public ScriptedAI
{
    npc_target_dummyAI(Creature *c) : ScriptedAI(c) {}
...
CreatureAI* GetAI_npc_target_dummy(Creature *_Creature)
{
    return new npc_target_dummyAI(_Creature);
}
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (SpellTimer < diff)
        {
            ...
            else //Venomous Snake
            {
                ...
                {
                    if(Owner)
                        Owner->CastSpell(m_creature->getVictim(), SPELL_DEADLY_POISON, true);
                }
