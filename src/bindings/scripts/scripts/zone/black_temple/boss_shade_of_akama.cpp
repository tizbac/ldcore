...
{
    {517.651, 400.921, 3.141592653, 112.783928},
...
struct TRINITY_DLL_DECL boss_shade_of_akamaAI : public ScriptedAI
{
    ...
    void JustDied(Unit* killer)
    {
        ...
        Creature * Akama = ObjectAccessor::GetCreature(*m_creature,AkamaGUID);
        if ( Akama )
        {
          Akama->CombatStop();
          Akama->RemoveAllAuras();
    ...
    void SummonCreature()
    {
        ...
        for (uint8 pos = 0; pos < 2; ++pos)
        {
            ...
            for (uint8 i = 0; i < 3; ++i)
            {
                Creature* Spawn = m_creature->SummonCreature(spawnEntries[i], X, Y, Z_SPAWN, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
                if (Spawn)
                {
    ...
    {
        //printf("Deathcount=%d\n",DeathCount);
        if ((!m_creature->getThreatManager().getThreatList().size() > 0) && DeathCount < 6)
            return;
        //printf("Threatlistsize=%d\n",m_creature->getThreatManager().getThreatList().size());
        if (IsBanished)
        {
            ...
            if (m_creature->getThreatManager().getThreatList().size() < 2)
            {
                ...
                Creature * Akama = ObjectAccessor::GetCreature(*m_creature,AkamaGUID);
                if (Akama )
                    m_creature->DealDamage(Akama,Akama->GetHealth());
                return;
            }
...
struct TRINITY_DLL_DECL npc_akamaAI : public ScriptedAI
{
    ...
    void EnterCombat(Unit* who) {
        m_creature->RemoveAurasDueToSpell(40447);
    }
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        ...
        case 0:
            ...
            m_creature->GetMotionMaster()->MovePoint(6, AkamaWP[1].x, AkamaWP[1].y, AkamaWP[1].z);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (ShadeHasDied && (WayPointId == 1))
        {
            ...
            printf("WayPoint %d\n");
            SoulRetrieveTimer = 1000000;
            m_creature->GetMotionMaster()->MovePoint(0, 517.65, 400.92, 112.783);
            ++WayPointId;
        }
        ...
        if (!ShadeHasDied && StartCombat)
        {
            if (CheckTimer < diff)
            {
                if (ShadeGUID)
                {
                    ...
                    if (Shade && !Shade->isAlive())
                    {
                        printf("Shade of Akama è schiattato\n");
                        ShadeHasDied = true;
                        WayPointId = 0;
                        m_creature->CombatStop();
                        ...
                        m_creature->InterruptNonMeleeSpells(false);
                        SoulRetrieveTimer = 1000000;
                        m_creature->GetMotionMaster()->MovePoint(0, 517.65, 400.92, 112.783928);
                    }
                    if (Shade && Shade->isAlive())
                    {
