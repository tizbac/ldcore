...
struct TRINITY_DLL_DECL npc_barnesAI : public npc_escortAI
{
    ...
    std::vector<uint64> summoned;
    uint32 TalkCount;
    uint32 TalkTimer;
    uint32 WipeTimer;
    uint32 TalkEmoteTimer;
    uint32 m_uiEventId;
    uint32 LockDoorsTimer;
    uint32 UnLockDoorsTimer;
    ...
    void ResetOpera()
    {
        ...
        if ( summoned.size() > 0)
        {
            ...
            for ( it = summoned.begin(); it != summoned.end(); it++)
            {
                if ( Creature * c = Creature::GetCreature(*m_creature,*it))
                    c->ForcedDespawn();
            }
            summoned.clear();
        }
        ...
        {
            m_creature->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);// Riabilita il gossip
    ...
    void Reset()
    {
        ...
        if (pInstance->GetData(DATA_OPERA_EVENT) != DONE )
        {
            LockDoorsTimer = 1000;// Altrimenti spawna prima lui delle porte e non le trova
            pInstance->SetData(DATA_OPERA_EVENT,NOT_STARTED);
        }
        else
            LockDoorsTimer = 0;
        PerformanceReady = false;
        UnLockDoorsTimer = 5000;
        if (pInstance)
    ...
    void WaypointReached(uint32 i)
    {
		...
		m_creature->RemoveAllAuras();// Toglie il vestito tuxedo
		ReturnToLastPoint();
                break;
        }
    }
	...
	if (pInstance)
	  pInstance->SetData(DATA_OPERA_EVENT,IN_PROGRESS);
		...
		summoned.push_back(pCreature->GetGUID());
            }
        }
	PerformanceReady = true;
        RaidWiped = false;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if ( LockDoorsTimer && (LockDoorsTimer <= diff ))
        {
            if (GameObject* Go = GameObject::GetGameObject((*m_creature), pInstance->GetData64(DATA_GAMEOBJECT_STAGEDOORLEFT)))
            {
            ...
            if (GameObject* Go = GameObject::GetGameObject((*m_creature), pInstance->GetData64(DATA_GAMEOBJECT_STAGEDOORRIGHT)))
            {
                Go->SetFlag(GAMEOBJECT_FLAGS,GO_FLAG_INTERACT_COND);
            }
            LockDoorsTimer = 0;
        ...
        {
            if (TalkEmoteTimer <= diff)
            {
                m_creature->HandleEmoteCommand(EMOTE_ONESHOT_TALK);
                TalkEmoteTimer = 0;
            }
            else TalkEmoteTimer-=diff;
        ...
        if (PerformanceReady)
        {
			...
			if (GameObject* Go = GameObject::GetGameObject((*m_creature), pInstance->GetData64(DATA_GAMEOBJECT_STAGEDOORLEFT)))
			{
			  ...
			  {
			    Go->RemoveFlag(GAMEOBJECT_FLAGS,GO_FLAG_INTERACT_COND); //Riapre la porta in caso di wip
			    Go->SetGoState(GO_STATE_READY);
			  }
			}
			ResetOpera();
        ...
        {
            if ( pInstance->GetData(DATA_OPERA_EVENT) == NOT_STARTED)
            {
                if (UnLockDoorsTimer <= diff)
                {
                    bool unlock = false;
                    Map* pMap = m_creature->GetMap();
                    if (pMap && !pMap->IsDungeon()) return;
