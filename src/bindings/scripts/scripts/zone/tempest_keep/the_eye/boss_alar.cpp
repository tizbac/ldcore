...
struct TRINITY_DLL_DECL boss_alarAI : public ScriptedAI
{
    ...
    uint32 RangedResetThreat_Timer;
   ...
   void JustDied(Unit *victim)
    {
        ...
        if(map->IsDungeon())
        {
            ...
            for (InstanceMap::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
            {
                ...
                {
                    if(i_pl->GetQuestStatus(10946) == QUEST_STATUS_INCOMPLETE && i_pl->HasAura(39527,0))
                        i_pl->CompleteQuest(10946);
                }
            }
        }
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(Phase1)
        {
            ...
            if(RangedResetThreat_Timer < diff)
            {
                 ...
                 for (std::list<HostilReference*>::iterator itr = threatlist.begin(); itr != threatlist.end(); ++itr)
                 {
                     ...
                     if(pUnit && DoGetThreat(pUnit) && !m_creature->IsWithinDistInMap(pUnit, 5))
    ...
    //            return;
            ...
            if(FlamePatch_Timer < diff)
            {
                ...
                FlamePatch_Timer = 30000+rand()%20000;
...
struct TRINITY_DLL_DECL mob_ember_of_alarAI : public ScriptedAI
{
    ...
    void DamageTaken(Unit* pKiller, uint32 &damage)
    {
        if(damage >= m_creature->GetHealth() && pKiller != m_creature && !toDie)
        {
            ...
            m_creature->SetDisplayId(11686);
            m_creature->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            //if(pInstance && pInstance->GetData(DATA_ALAREVENT) == IN_PROGRESS)
