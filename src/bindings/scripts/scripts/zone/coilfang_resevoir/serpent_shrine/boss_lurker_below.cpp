...
#define SPELL_WHIRL         34109 //37660
...
//Guardian spells
#define SPELL_ARCINGSMASH   39144     //38761 // Wrong SpellId. Can't find the right one.
...
struct TRINITY_DLL_DECL boss_the_lurker_belowAI : public Scripted_NoMovementAI
{
    ...
    {
        if (!who || m_creature->getVictim() || RotType!=NOROTATE || Submerged)
            return;
    ...
    void EnterCombat(Unit *who)
    {
        ...
        if (!who || m_creature->getVictim() || Submerged || RotType!=NOROTATE)
            return;
    ...
    void JustDied(Unit* Killer)
    {
        ...
        if (map->IsDungeon() )
        {
            ...
            for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
            {
                ...
                //Scalding water
                if(target && target->HasAura(37284,0))
        ...
        if (map->IsDungeon())
        {
            ...
            for(Map::PlayerList::const_iterator i = PlayerList.begin();i != PlayerList.end(); ++i)
            {
                if(Player* pPlayer = i->getSource())
                {
                    ...
                    if (pPlayer->GetDistance(m_creature) < 1.5 && pPlayer->IsInWater() && pPlayer->GetBaseSkillValue(SKILL_FISHING) >= 375)
                    {
    ...
    {
        //NO aggro e no target check di player in range.
        ...
        //Timer per controllo immersione.
        if(!Submerged)
        {
            if(PhaseTimer < diff)
            {
            ...
            if(RotType == NOROTATE)
            {
                ...
                //Waterbolt se nessuno in range melee
                if(WaterboltTimer < diff)
                {
                    ...
                    if(target && !m_creature->IsWithinDistInMap(target, 2))
                ...
                //Controllo sput timer
                if(SpoutTimer < diff)
                {
                    if(m_creature->getVictim())
                        StartRotate(m_creature->getVictim());
                    SpoutTimer= 35000;
                }
                else
                    SpoutTimer -= diff;
