    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(ShapeShift_Timer < diff)
        {
            ...
            else
            {
                ...
                LaceratingSlash_Timer = 7000; // dur 18s
                RendFlesh_Timer = 5000;  // dur 5s
        ...
        if(!inBearForm)
        {
            ...
            if(Mangle_Timer < diff)
            {
                ...
                m_creature->RemoveAurasDueToSpell(SPELL_MANGLEEFFECT);
                DoCast(m_creature->getVictim(), SPELL_MANGLEEFFECT, true);
            ...
            if(Surge_Timer < diff)
            {
                ...
                Unit* new_target = SelectUnit(SELECT_TARGET_RANDOM, 1, 45, true);
                Unit* old_target = m_creature->getVictim();
                ...
                TankGUID = old_target->GetGUID();
                ChargeTargetGUID = new_target->GetGUID();

                float x, y, z;
                new_target->GetContactPoint(m_creature, x, y, z);
