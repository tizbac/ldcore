...
static SHostageInfo HostageInfo[] =
{
    ...
    {24024, 186667, -35, 1134, 18.71, 1.9}, // dragonhawk
    {24001, 186672, 413, 1117,  6.32, 3.1}  // lynx
...
struct TRINITY_DLL_DECL instance_zulaman : public ScriptedInstance
{
    ...
    void Initialize()
    {
        ...
        uint64 HalazziEntranceDoorGUID = 0;
        uint64 AkilzonDoorGUID = 0;
        uint64 HalazziExitDoorGUID = 0;
        uint64 ZulJinDoorGUID = 0;
        uint64 ZulAmanMainGateGUID = 0;
    ...
    void OnObjectCreate(GameObject *go)
    {
        ...
        case 186728:
            ZulAmanMainGateGUID = go->GetGUID();
    ...
    {
        if(SacreficeEventStarted == 1)
            OpenDoor(ZulAmanMainGateGUID, true);
    ...
    const char* Save()
    {
        ...
        ss << "Z A " << BossKilled << " " << ChestLooted << " " << QuestMinute << " " << SacreficeEventStarted
            << " " << ChestLootTyp[0] << " " << ChestLootTyp[1] << " " << ChestLootTyp[2] << " " << ChestLootTyp[3];
    ...
    void Load(const char* load)
    {
        ...
        uint16 data1, data2, data3, data5, data6, data7, data8;
        uint32 data4;
        ss >> dataHead1 >> dataHead2 >> data1 >> data2 >> data3 >> data4 >> data5 >> data6 >> data7 >> data8;
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        case DATA_HALAZZIEVENT:
            ...
            {
            case IN_PROGRESS:
                OpenDoor(HalazziEntranceDoorGUID, false);
                break;
            case NOT_STARTED: 
                OpenDoor(HalazziEntranceDoorGUID, true);
                OpenDoor(HalazziExitDoorGUID, false);
                break;
            case DONE:
                OpenDoor(HalazziEntranceDoorGUID, true);
        ...
        if(data == DONE && type <= DATA_ZULJINEVENT)
        {
            BossKilled++;
