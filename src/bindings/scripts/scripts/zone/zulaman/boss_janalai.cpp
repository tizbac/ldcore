...
float JanalaiSpawn[1][4] =
{
    {-18.6421,1150.46,18.7947,3.19395}
};
...
struct TRINITY_DLL_DECL boss_janalaiAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        m_creature->SetHomePosition(JanalaiSpawn[0][0],JanalaiSpawn[0][1],JanalaiSpawn[0][2],JanalaiSpawn[0][3]);
    ...
    void FireWall()
    {
        ...
        for(uint8 i = 0; i < 4; i++)
        {
            wall = m_creature->SummonCreature(21252,FireWallCoords[i][0],FireWallCoords[i][1],FireWallCoords[i][2],FireWallCoords[i][3],TEMPSUMMON_TIMED_DESPAWN,15000);
            //wall = m_creature->SummonTrigger(FireWallCoords[i][0],FireWallCoords[i][1],FireWallCoords[i][2],FireWallCoords[i][3],15000);
            if(wall)
            {
    ...
    bool HatchAllEggs(uint32 action) //1: reset, 2: isHatching all
    {
...

        {
            ...
            Trinity::AllCreaturesOfEntryInRange check(m_creature, MOB_EGG, 200);
    ...
    void HandleHatcherTimer(uint32 diff)
    {
        if(!noeggs)
        {
    ...
    void HandelEnrageTimer(uint32 diff)
    {
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if(isFlameBreathing)
        {
            ...
            {
                HandleHatcherTimer(diff);
                HandelEnrageTimer(diff);
                return;
            }
        }
        ...
        if(BombTimer < diff)
        {
            ...
            for(Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
            {
                if (Player* i_pl = i->getSource())
                    ...
                    {
                        float x, y, z;
                        i_pl->GetPosition(x, y, z);
                        if(x < -70 || x > 0 || y > 1173 || y < 1124 || z < 18)
...
struct TRINITY_DLL_DECL mob_janalai_firebombAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff) 
    {
        if(boom_timer < diff)
        {
            m_creature->CastSpell(m_creature, SPELL_FIRE_BOMB_DAMAGE, true);
