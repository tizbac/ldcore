...
#define SPELL_STATIC_VISUAL         43622
...
struct TRINITY_DLL_DECL boss_akilzonAI : public ScriptedAI
{
    ...
    {
        if(spell->Id == SPELL_STATIC_VISUAL)
            m_creature->CastSpell(target,SPELL_STATIC_DISRUPTION,true);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (StaticDisruption_Timer < diff)
        {
            ...
            {
                Unit* target = SelectUnit(SELECT_TARGET_RANDOM, 1, 300, true);
                if(!target) target = m_creature->getVictim();
                TargetGUID = target->GetGUID();

                SpellEntry *spellInfo = (SpellEntry*)GetSpellStore()->LookupEntry(SPELL_STATIC_VISUAL);
                if(spellInfo)
                {
                    spellInfo->EffectTriggerSpell[0] = 0;
                    m_creature->CastSpell(target, spellInfo, false);
