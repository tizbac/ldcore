...
void World::RecordTimeDiff(const char *text, ...)
{
    ...
    if(diff > m_configs[CONFIG_MIN_LOG_UPDATE])
    {
        ...
        sLog.outArena("Difftime %s: %u.", str, diff);
    }
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= m_gameTime))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
            {
                ...
                cell.Visit(p, object_checker, *MapManager::Instance().GetMap(summoner->GetMapId(), summoner));
            ...
            case SCRIPT_COMMAND_OPEN_DOOR:
            {
                ...
                TypeContainerVisitor<Trinity::GameObjectSearcher<Trinity::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
                cell.Visit(p, object_checker, *MapManager::Instance().GetMap(caster->GetMapId(), (Unit*)source));
