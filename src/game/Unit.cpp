...
Unit::~Unit()
{
    ...
    if(pthread_mutex_destroy(&mid) != 0)
        sLog.outError("Mutex locket during Unit destroy");
...
bool Unit::HasAurasWithInterruptFlags(uint32 flag)
{
    ...
    for (iter = m_interruptableAuras.begin(); iter != m_interruptableAuras.end(); )
    {
        ...
        if(aur && (aur->GetSpellProto()->AuraInterruptFlags & flag) && !aur->IsPositive())
        {
            if(aur->GetCaster() && aur->GetCaster()->GetTypeId() == TYPEID_PLAYER)
                return true;
        }
    }
...
void Unit::RemoveSpellbyDamageTaken(uint32 damage, uint32 spell)
{
    ...
    for(i = m_ccAuras.begin(); i != m_ccAuras.end(); i = next)
    {
        ...
        {
            if( !((*i)->GetId()==12494 && (*i)->SkipBreakChance()) )
            {
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    uint32 calcDamage = damage;
    if(cleanDamage && cleanDamage->damage > damage)
        calcDamage = cleanDamage->damage;

    if (calcDamage)
    {
       // interrupting auras with AURA_INTERRUPT_FLAG_DAMAGE before checking !damage (absorbed damage breaks that type of auras)
       pVictim->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_DAMAGE, spellProto ? spellProto->Id : 0);
       pVictim->RemoveSpellbyDamageTaken(calcDamage, spellProto ? spellProto->Id : 0);
    }
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage *damageInfo, int32 damage, SpellEntry const *spellInfo, WeaponAttackType attackType, bool crit)
{
    ...
    {
        uint32 armorReducedDamage = CalcArmorReducedDamage(pVictim, damage);
        damageInfo->cleanDamage += damage - armorReducedDamage;
    ...
    damageInfo->cleanDamage += damageInfo->absorb + damageInfo->resist;
}
...
void Unit::AttackerStateUpdate (Unit *pVictim, WeaponAttackType attType, bool extra )
{
    ...
    ProcDamageAndSpell(damageInfo.target, damageInfo.procAttacker, damageInfo.procVictim, damageInfo.procEx, damageInfo.damage, damageInfo.cleanDamage, damageInfo.attackType);
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit *pVictim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance, bool SpellCasted ) const
{
    ...
    if (pVictim->GetTypeId() == TYPEID_PLAYER && !pVictim->HasInArc(M_PI/2,this))
    {
...
SpellMissInfo Unit::SpellHitResult(Unit *pVictim, SpellEntry const *spell, bool CanReflect)
{
    ...
    if(spell->Mechanic == MECHANIC_IMMUNE_SHIELD && pVictim->HasAura(33786, 0))
...
bool Unit::AddAura(Aura *Aur)
{
    ...
    if (!Aur->IsPassive() && !Aur->IsPersistent())
    {
        ...
        {
            if(i2->second->GetCasterGUID()==Aur->GetCasterGUID() || Aur->IsForceStack())
            {
                if(Aur->IsDualStack() && Aur->GetCastItemGUID() && Aur->GetCastItemGUID() != i2->second->GetCastItemGUID())
                {
                    Aur->SetActiveDualStack(true);
                    ++i2;
                    continue;
                }
                else
                {
                    Aur->SetActiveDualStack(false);
                    if (!stackModified)
                    {
...
void Unit::RemoveSingleAuraFromStackByDispel(uint32 spellId, uint64 casterGUID)
{
    for (AuraMap::iterator iter = m_Auras.begin(); iter != m_Auras.end(); )
    {
        Aura *aur = iter->second;
...
void Unit::RemoveAura(AuraMap::iterator &i, AuraRemoveMode mode)
{
    ...
    m_Auras.erase(i);
    ++m_removedAurasCount;
    SpellEntry const* AurSpellInfo = Aur->GetSpellProto();                                      // internal count used by unit update
    Unit* caster = NULL;
    Aur->UnregisterSingleCastAura();
...
void Unit::ProcDamageAndSpell(Unit *pVictim, uint32 procAttacker, uint32 procVictim, uint32 procExtra, uint32 amount, uint32 cleanAmount, WeaponAttackType attType, SpellEntry const *procSpell, bool canTrigger)
{
     // Not much to do if no flags are set.
    if (procAttacker && canTrigger)
        ProcDamageAndSpellFor(false,pVictim,procAttacker, procExtra,attType, procSpell, amount, cleanAmount);
    // Now go on with a victim's events'n'auras
    // Not much to do if no flags are set or there is no victim
    if(pVictim && pVictim->isAlive() && procVictim)
        pVictim->ProcDamageAndSpellFor(true,this,procVictim, procExtra, attType, procSpell, amount, cleanAmount);
}
...
bool Unit::HandleDummyAuraProc(Unit *pVictim, uint32 damage, uint32 cleanDamage, Aura* triggeredByAura, SpellEntry const * procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            switch(dummySpell->Id)
            {
                ...
                case 12848:
                {
                    ...
                    if (pVictim && pVictim->HasAura(12654, 0))
                   {
                        Unit::AuraList const &mPeriodic = pVictim->GetAurasByType(SPELL_AURA_PERIODIC_DAMAGE);
                        for(Unit::AuraList::const_iterator i = mPeriodic.begin(); i != mPeriodic.end(); ++i)
                        {
                           if( (*i)->GetSpellProto()->Id == 12654)
                            {
                                int32 formerIgniteBasePoints = (*i)->GetBasePoints();
                                if ((*i)->GetAuraDuration() > (*i)->GetAuraMaxDuration()/2)
                                    basepoints0 += formerIgniteBasePoints;
                                else
                                    basepoints0 += int32(formerIgniteBasePoints / 2);
                                break;
                            }
                        }
                    }
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch(dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    //  totem astral winds
                    int32 APbonusTotem = 0;
                    Player* player = (Player*)this;
                    Item* totem = player->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_RANGED);
                    if(totem and totem->GetEntry() == 27815)
                    APbonusTotem = 80;
                    ...
                    if ( castItem->GetSlot() == EQUIPMENT_SLOT_OFFHAND )
                    {
                        ...
                        basepoints0 = int32((extra_attack_power + APbonusTotem)/14.0f * GetAttackTime(OFF_ATTACK)/1000/2);
                    ...
                    {
                        // Value gained from additional AP
                        basepoints0 = int32((extra_attack_power + APbonusTotem)/14.0f * GetAttackTime(BASE_ATTACK)/1000);
...
uint32 Unit::SpellHealingBonus(SpellEntry const *spellProto, uint32 healamount, DamageEffectType damagetype, Unit *pVictim)
{
    ...
    if(fixedAmount)
    {
        float finalHeal = healamount + 0.0f;
        float bonusHeal = 0;
        if(healingReduction)
        {
            bonusHeal = pVictim->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_HEALING_PCT);
            if(bonusHeal)
                finalHeal *= (100.0f + bonusHeal) / 100.0f;
        }

        if(pVictim->GetTypeId()==TYPEID_PLAYER && pVictim->getClass()==CLASS_WARLOCK && checkSpecialBonus)
        {
            float bonusTalent = 1;
            bonusHeal = 0;
...
bool Unit::isVisibleForOrDetect(Unit const* u, bool detect, bool inVisibleList, bool is3dDistance) const
{
    ...
    return (u->canSeeOrDetect(this, detect, inVisibleList, is3dDistance) && u->canDetectStealthOf(this, u->GetDistance(this)));
}
...
bool Unit::canDetectStealthOf(Unit const* target, float distance) const
{
    ...
    float visibleDistance = GetTypeId() == TYPEID_PLAYER ? 14.0f : 9.0f;

    float stealthLevel = (target->GetTotalAuraModifier(SPELL_AURA_MOD_STEALTH)/5.0f);
    float enemyLevel   = float(getLevelForTarget(target));

    int32 enemyDetection = GetTotalAuraModifier(SPELL_AURA_MOD_DETECT);
    if(enemyDetection<0)
        enemyDetection = 0;
    ...
    if(GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_WARRIOR)
    {
        ...
        switch(group)
        {
            ...
            case DIMINISHING_TRIGGER_ROOT:
            {
                ...
                {
                    //Rank 2
                    if(itr->second->GetId() == 29838)
                    {
                        CastSpell(this, 29842, true);
                        break;
                    }
                    //Rank 1
                    if(itr->second->GetId() == 29834)
                    {
...
void Unit::ProcDamageAndSpellFor( bool isVictim, Unit * pTarget, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellEntry const * procSpell, uint32 damage, uint32 cleanDamage )
{
    ++m_procDeep;
    if (m_procDeep > 5)
    {
    ...
    for(AuraMap::const_iterator itr = GetAuras().begin(); itr!= GetAuras().end(); ++itr)
    {
        ...
        for(int i=0;i<3;++i)
        {
            uint32 triggered_spell_id = itr->second->GetSpellProto()->EffectTriggerSpell[i];
            bool positive = (!triggered_spell_id && IsPositiveSpell(itr->second->GetId()) && itr->second->m_procCharges > 0) ||
                            (!sSpellStore.LookupEntry(triggered_spell_id) && IsPositiveSpell(itr->second->GetId())) ||
        ...
        if(procSpell)
        {
            switch(itr->second->GetId())
            {
                ...
                case 12328:
                    ...
                    {
                        if(pTarget != this->getVictim())
                            skip = true;
                        break;
                    }
                    if(procSpell->Id == 20647)
                    {
                        skip = true;
                        break;
                    }
...
//                    sLog.outDebug("     <end of list>");
        ...
        switch(auraModifier->m_auraname)
        {
            ...
            case SPELL_AURA_DUMMY:
            {
                ...
                if (!HandleDummyAuraProc(pTarget, damage, cleanDamage, triggeredByAura, procSpell, procFlag, procExtra, cooldown))
                    continue;
                break;
            }
            case SPELL_AURA_MOD_HASTE:
...
void Unit::Kill(Unit *pVictim, bool durabilityLoss)
{
    ...
    if(bRewardIsAllowed && player && player!=pVictim)
    {
        if(player->RewardPlayerAndGroupAtKill(pVictim))
            player->ProcDamageAndSpell(pVictim, PROC_FLAG_KILL_AND_GET_XP, PROC_FLAG_KILLED, PROC_EX_NONE, 0, 0);
        else
            player->ProcDamageAndSpell(pVictim, PROC_FLAG_NONE, PROC_FLAG_KILLED,PROC_EX_NONE, 0, 0);
    }
