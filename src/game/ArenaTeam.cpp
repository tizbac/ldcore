...
bool ArenaTeam::Create(uint64 captainGuid, uint32 type, std::string ArenaTeamName)
{
    ...
    //ARENA_LOG
    CharacterDatabase.PExecute("INSERT INTO arena_log (action,a_teamid,player_source) "
        "VALUES('%u','%u','%u')", 1, Id, GUID_LOPART(CaptainGuid));
...
/*
//TABELLA DA INSERIRE NEL DB CHARACTERS


CREATE TABLE `arena_log` (
	`time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
	`action` int(11) UNSIGNED NOT NULL DEFAULT '0',
	`a_teamid` int(10) UNSIGNED NOT NULL DEFAULT '0',
	`a_dmg` int(10) UNSIGNED NOT NULL DEFAULT '0',
	`a_heal` int(10) UNSIGNED NOT NULL DEFAULT '0',
	`a_plrcount` int(10) UNSIGNED NOT NULL DEFAULT '0',
	`b_teamid` int(11) UNSIGNED NOT NULL DEFAULT '0',
	`b_dmg` int(11) UNSIGNED NOT NULL DEFAULT '0',
	`b_heal` int(11) UNSIGNED NOT NULL DEFAULT '0',
	`b_plrcount` int(11) UNSIGNED NOT NULL DEFAULT '0',
	`winner` int(11) UNSIGNED NOT NULL DEFAULT '0',
	`rating_change` int(11) UNSIGNED NOT NULL DEFAULT '0',
	`player_source` int(11) UNSIGNED NOT NULL DEFAULT '0',
	`player_dest` int(11) UNSIGNED NOT NULL DEFAULT '0'
) ENGINE=`InnoDB` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ROW_FORMAT=COMPACT CHECKSUM=0 DELAY_KEY_WRITE=0;
*/
    //sLog.outArena("{CREATING TEAM [Id: %u | Name: %s | Type: %u | Captain GUID: %u]}", GetId(), ArenaTeamName.c_str(), GetType(), GetCaptain());
    return true;
}
...
bool ArenaTeam::AddMember(const uint64& PlayerGuid)
{
    ...
    CharacterDatabase.PExecute("INSERT INTO arena_log (action,a_teamid,player_source) "
        "VALUES('%u','%u','%u')", 2, Id, GUID_LOPART(newmember.guid));
    CharacterDatabase.CommitTransaction();
    ...
    if(pl)
    {
        ...
        //sLog.outArena("{PLAYER [Player: %s | GUID: %u] JOIN TEAM [type: %u | Id: %u]}", pl->GetName(), pl->GetGUIDLow(), GetType(), GetId());
    }
    return true;
}
...
void ArenaTeam::SetCaptain(const uint64& guid)
{
    ...
    {
        //ARENA_LOG
        CharacterDatabase.PExecute("INSERT INTO arena_log (action,a_teamid,player_source,player_dest) "
            "VALUES('%u','%u','%u','%u')", 3, Id, oldcaptain->GetGUIDLow(), GUID_LOPART(guid));
    }
    CharacterDatabase.CommitTransaction();
...
void ArenaTeam::DelMember(uint64 guid)
{
    ...
    //ARENA_LOG
    CharacterDatabase.PExecute("INSERT INTO arena_log (action,a_teamid,player_source) "
        "VALUES('%u','%u','%u')", 4, GetId(), GUID_LOPART(guid));
    CharacterDatabase.CommitTransaction();
}
...
void ArenaTeam::Disband(WorldSession *session)
{
    ...
    if(Player *player = session->GetPlayer())
    {
        //ARENA_LOG
        CharacterDatabase.PExecute("INSERT INTO arena_log (action,a_teamid,player_source) "
            "VALUES('%u','%u','%u')", 5, Id, player->GetGUIDLow());
