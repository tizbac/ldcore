...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    {
        GetPlayer()->ForceReturnHome();
        LogoutPlayer(false);
        return;
    }

    GetPlayer()->AntiCheatUpdateIgnoreTimer();
    GetPlayer()->AntiCheatSetTeleport(true);
...
void WorldSession::HandleMovementOpcodes( WorldPacket & recv_data )
{
    ...
    uint32 time_diff   = time_now - _player->AntiCheatGetLastTime();
    uint32 ignore_diff = time_now - _player->AntiCheatGetIgnoreTimer();
    uint32 reset_diff  = time_now - _player->AntiCheatGetResetKickTimer();
    bool skip = false;
    bool removeMount = false;

    //Controllo condizioni di skip
    if( _player->AntiCheatGetLastTime() == 0 || time_diff == 0  || ignore_diff<3000 || _player->GetCharm() || (MovementFlags & (MOVEMENTFLAG_JUMPING | MOVEMENTFLAG_FALLING | MOVEMENTFLAG_ONTRANSPORT)))
        skip = true;

    if(!skip)
    {
        float diff_x = _player->AntiCheatGetLastPosX() - movementInfo.x;
        float diff_y = _player->AntiCheatGetLastPosY() - movementInfo.y;

        diff_x *= diff_x;
        diff_y *= diff_y;

        float diff_move = sqrt((diff_x+diff_y));
        float lag_mod = 1.0f;
        
        if(time_diff>500)
        {
            lag_mod = time_diff/500.0;
            diff_move /= lag_mod;
        }

        UnitMoveType mtype = MOVE_RUN;

        if(_player->HasAuraType(SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED) || _player->HasAuraType(SPELL_AURA_FLY))
        {
            mtype = MOVE_FLIGHT;
            removeMount = true;
        }
        else if(_player->IsInWater() && MovementFlags & (MOVEMENTFLAG_SWIMMING))
        {
            mtype = MOVE_SWIM;
        }

        if( (diff_move - lag_mod ) > _player->GetSpeed(mtype))
        {
            if(_player->AntiCheatIsTeleport())
            {
                _player->AntiCheatSetTeleport(false);
            }
            else
            {
                if(diff_move > 30 || _player->AntiCheatGetKickCount() > 3)
                {
                    //sLog.outCheat("Player[%s] GUID[%u] Speed[%f] MaxSpeed[%f] TimeDiff[%u] Count[KICKED]", _player->GetName(), _player->GetGUIDLow(), diff_move, _player->GetSpeed(mtype), time_diff);
                    KickPlayer();
                    return;
                }
                
                //sLog.outCheat("Player[%s] GUID[%u] Speed[%f] MaxSpeed[%f] TimeDiff[%u] Count[%u]", _player->GetName(), _player->GetGUIDLow(), diff_move, _player->GetSpeed(mtype), time_diff, _player->AntiCheatGetKickCount());
                _player->AntiCheatSetResetKickTimer();
                _player->AntiCheatKickCount();
            }
        }
        else
        {
            if(_player->AntiCheatGetKickCount() !=0 && reset_diff > 90000)
            {
                _player->AntiCheatResetKickCount();
                //sLog.outCheat("Player[%s] GUID[%u] clean reset count [%u]", _player->GetName(), _player->GetGUIDLow(), _player->AntiCheatGetKickCount());
            }
        }
    }

    _player->AntiCheatUpdateLastPos();
    _player->AntiCheatUpdateLastTime();
...
void WorldSession::HandlePossessedMovement(WorldPacket& recv_data, MovementInfo& movementInfo, uint32& MovementFlags)
{
    ...
    if (pos_unit->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        float fBreakChance = 0.0f;
        if(!GetPlayer()->IsWithinLOSInMap(pos_unit))
            fBreakChance+=20;

        fBreakChance += (GetPlayer()->GetDistance(pos_unit))*0.2;

        if(roll_chance_f(fBreakChance))
            GetPlayer()->Uncharm();
    }
...
void WorldSession::HandleForceSpeedChangeAck(WorldPacket &recv_data)
{
    ...
    _player->AntiCheatSetTeleport(true);
    _player->AntiCheatUpdateIgnoreTimer();
