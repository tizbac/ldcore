...
void Spell::EffectInstaKill(uint32 /*i*/)
{
    ...
    if(unitTarget->GetTypeId() == TYPEID_PLAYER)
        sLog.outArena("Unit [GUID:%u] cast instant kill on  Player [GUID:%u]", m_caster->GetGUIDLow(), unitTarget->GetGUIDLow());
            ...
            case SPELLFAMILY_GENERIC:
            {
                ...
                switch(m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    case 33671:
                    {
                        ...
                        if(unitTarget->GetGUID() == m_caster->GetGUID() || unitTarget->GetTypeId() != TYPEID_PLAYER || unitTarget->isDead())
                            return;
...
void Spell::EffectTeleportUnits(uint32 i)
{
    ...
    if(unitTarget->GetTypeId() == TYPEID_PLAYER)
    {
        ((Player*)unitTarget)->AntiCheatSetTeleport(true);
        ((Player*)unitTarget)->AntiCheatUpdateIgnoreTimer();
...
void Spell::EffectEnergize(uint32 i)
{
    ...
    // 10052 Mana Jade, 5405 Mana Agate, 10057 Mana Citrine, 10058 Mana Ruby, 27103 Mana Emerald
    if ( m_spellInfo->Id == 10052 or m_spellInfo->Id == 5405 or m_spellInfo->Id == 10057 or m_spellInfo->Id == 10058 or m_spellInfo->Id == 27103 )
    {
        Player* player = (Player*)m_caster;
        Item* trinket1 = player->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_TRINKET1);
        Item* trinket2 = player->GetItemByPos(INVENTORY_SLOT_BAG_0,EQUIPMENT_SLOT_TRINKET2);
        if ((trinket1 and trinket1->GetEntry() == 30720) or ( trinket2 and trinket2->GetEntry() == 30720))
        {
            player->CastSpell(m_caster,37445,true); // Casta mana surge
        }
    }
        ...
        if (!success_list.empty())
        {
            ...
            for (std::list<std::pair<uint32,uint64> >::iterator j = success_list.begin(); j != success_list.end(); ++j)
            {
                ...
                if(spellInfo->StackAmount!= 0)
                {
                    ...
                    unitTarget->RemoveSingleAuraFromStackByDispel(spellInfo->Id, j->second);
                }
                else
...
void Spell::EffectEnchantItemTmp(uint32 i)
{
    ...
    uint32 enchant_id = m_spellInfo->EffectMiscValue[i];
    float setBonusExtra = 1.0f;
    switch(enchant_id)
    {
        case 5:      //Weapon 1   lvl 10
        case 4:      //Weapon 2   lvl 18
        case 3:      //Weapon 3   lvl 26
        case 523:    //Weapon 4   lvl 36
        case 1665:   //Weapon 5   lvl 46
        case 1666:   //Weapon 6   lvl 56
        case 2637:   //Weapon 7   lvl 64
        {
            Unit::AuraMap const& uAuras = m_caster->GetAuras();
            for (Unit::AuraMap::const_iterator itr = uAuras.begin(); itr != uAuras.end(); ++itr)
            {
                //Elemental Weapons Rank 1
                if(itr->second->GetId() == 16266)
                {
                    setBonusExtra = 1.05f;
                    break;
                }
                //Elemental Weapons Rank 2
                else if(itr->second->GetId() == 29079)
                {
                    setBonusExtra = 1.10f;
                    break;
                }
                //Elemental Weapons Rank 3
                else if(itr->second->GetId() == 29080)
                {
                    setBonusExtra = 1.15f;
                    break;
                }
    ...
    if(setBonusExtra > 1.0f)
        itemTarget->SetEnchantBonus(setBonusExtra);
}
...
void Spell::SpellDamageWeaponDmg(uint32 i)
{
    ...
    if(fixed_bonus || spell_bonus)
    {
        ...
        if(fixed_bonus)
        {
            if(m_attackType == OFF_ATTACK)
                fixed_bonus = fixed_bonus * offhand_bonus;
    ...
    if(spell_bonus)
    {
        float bonus_holy_pct = 1;
        weaponDamage += spell_bonus;

        if(m_caster->HasAura(20218, 0))
            bonus_holy_pct += 0.1;

        Unit::AuraList const& mTriggerAuras = m_caster->GetAurasByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
        for(Unit::AuraList::const_iterator i = mTriggerAuras.begin(); i != mTriggerAuras.end(); ++i)
            if( ( (*i)->GetSpellProto()->SpellFamilyName == SPELLFAMILY_PALADIN ) && ( (*i)->GetSpellProto()->SpellIconID == 84 ) )
                bonus_holy_pct += ((*i)->GetStackAmount() * (*i)->GetModifier()->m_amount)/100.0;

        weaponDamage *= bonus_holy_pct;
    }
...
void Spell::EffectInterruptCast(uint32 j)
{
    if(!unitTarget)
        return;
    if(!unitTarget->isAlive())
    ...
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
    {
        if (unitTarget->m_currentSpells[i])
        {
            ...
            if ( (unitTarget->m_currentSpells[i]->getState() == SPELL_STATE_CASTING || (unitTarget->m_currentSpells[i]->getState() == SPELL_STATE_PREPARING && unitTarget->m_currentSpells[i]->GetCastTime() > 0.0f)) && unitTarget->m_currentSpells[i]->m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT && unitTarget->m_currentSpells[i]->m_spellInfo->PreventionType == SPELL_PREVENTION_TYPE_SILENCE )
            {
                if(m_originalCaster)
                {
                    int32 duration = m_originalCaster->CalculateSpellDuration(m_spellInfo, j, unitTarget);
...
void Spell::EffectAddComboPoints(uint32 /*i*/)
{
    ...
    if(m_spellInfo->Id == 14157)
    {
        if(!((Player*)m_caster)->GetComboPointsDelayed())
            ((Player*)m_caster)->AddComboPointsDelayed(true);
    }
    else
...
void Spell::EffectStuck(uint32 /*i*/)
{
    ...
    pTarget->AntiCheatSetTeleport(true);
    pTarget->AntiCheatUpdateIgnoreTimer();
...
void Spell::EffectMomentMove(uint32 i)
{
    ...
    float fDist = GetSpellRadius(m_spellInfo,i,false);
    float fMove = 0.0;
    float fTempX, fTempY, fTempZ;
    float fDestX, fDestY, fDestZ;
    unitTarget->GetPosition(fDestX, fDestY, fDestZ);
    
    float fAngle = unitTarget->GetOrientation();
    float fFloor     = unitTarget->GetMap()->GetHeight(fDestX, fDestY, fDestZ, true);
    float fGround    = unitTarget->GetMap()->GetHeight(fDestX, fDestY, MAX_HEIGHT);
    float fSafeZ     = fFloor > fGround ? fFloor : fGround;
    float fStartZ    = fDestZ;
    bool  bCanFly    = true;
    bool  bMoveError = false;

    if( fabs(fDestZ-fSafeZ) < 2)
    {
        bCanFly = false;
        fDestZ  = fSafeZ;
    }

    for(float i=2.0f; i<fDist; i+=2.0f)
    {
        unitTarget->GetNearPoint2D(fTempX, fTempY, i, fAngle);

        fFloor  = unitTarget->GetMap()->GetHeight(fTempX, fTempY, fDestZ, true);
        fGround = unitTarget->GetMap()->GetHeight(fTempX, fTempY, MAX_HEIGHT);
        fTempZ = fFloor > fGround ? fFloor : fGround;

        if(fFloor <= INVALID_HEIGHT  && bCanFly)
        {
            if( fDestZ < fGround)
            {
                if( i < 3.0 )
        ...
        if(!unitTarget->IsWithinLOS(fTempX, fTempY, fTempZ))
        {
            if( i < 3.0 )
                bMoveError = true;
            break;
        ...
        fMove += sqrt(( (fDestX-fTempX) * (fDestX-fTempX) ) + ( (fDestY-fTempY) * (fDestY-fTempY) ) + ( (fDestZ-fTempZ) * (fDestZ-fTempZ)));
        if( fMove > fDist )
        {
            if( i < 3.0 )
                bMoveError = true;
    ...
    if(unitTarget->GetTypeId() == TYPEID_PLAYER)
    {
        ((Player*)unitTarget)->AntiCheatSetTeleport(true);
        ((Player*)unitTarget)->AntiCheatUpdateIgnoreTimer();
        ((Player*)unitTarget)->TeleportTo(unitTarget->GetMapId(), fDestX, fDestY, fDestZ, unitTarget->GetOrientation(), TELE_TO_NOT_LEAVE_COMBAT | TELE_TO_NOT_UNSUMMON_PET | (unitTarget==m_caster ? TELE_TO_SPELL : 0));
    }
    else
        unitTarget->GetMap()->CreatureRelocation((Creature*)unitTarget, fDestX, fDestY, fDestZ,unitTarget->GetOrientation());
...
void Spell::EffectCharge(uint32 /*i*/)
{
    ...
    if(m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ((Player *)m_caster)->AntiCheatSetTeleport(true);
        ((Player *)m_caster)->AntiCheatUpdateIgnoreTimer();
    }

    m_caster->GetMotionMaster()->MoveCharge(fNearX, fNearY, fNearZ);
