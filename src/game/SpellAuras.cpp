...
m_skipBreakChance(true), m_periodicTimer(0), m_amplitude(0), m_PeriodicEventId(0), m_AuraDRGroup(DIMINISHING_NONE),
m_canDualStack(false), m_forceSingleStack(false), m_tickNumber(0)
{
    assert(target);
	...
	case 25225:
        ...
        case 15258:
            m_canDualStack      = false;
            m_forceSingleStack  = true;
            m_activeDualStack   = false;
            break;
        default:
            m_activeDualStack   = false;
...
void Aura::_AddAura()
{
    ...
    for(uint8 i = 0; i < 3; i++)
    {
        ...
        for(Unit::AuraMap::const_iterator itr = m_target->GetAuras().lower_bound(spair); itr != m_target->GetAuras().upper_bound(spair); ++itr)
        {
            ...
            {
                if(m_activeDualStack && GetCastItemGUID() && GetCastItemGUID() != itr->second->GetCastItemGUID())
                {
                    //printf("SpellAuras: DualStack");
                }
                else
                {
                    secondaura = true;
    ...
    switch(m_spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if ( GetSpellProto()->SpellFamilyFlags & 0x1000000000LL )
            {
                ...
                {
                    if ( (GetAuraDuration() <= 0 || m_removeMode==AURA_REMOVE_BY_DISPEL) && GetStackAmount() > 0 )
                    {
                        // final heal
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (m_spellProto->SpellFamilyFlags & 0x0000000000000020LL)
            {
                ...
                if (apply && !loading && caster)
                {
                    ...
                    m_modifier.m_amount+=int32(((mwb_min+mwb_max)/2+ap*mws/14000)*GetSpellDuration(m_spellProto)*0.00743f/1000);
                }
                return;
            }
            break;
...
void Aura::PeriodicTick()
{
    ...
    switch(m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            pCaster->ProcDamageAndSpell(target, procAttacker, procVictim, procEx, pdamage, 0, BASE_ATTACK, spellProto);
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            {
                float tempDamage = m_target->GetMaxHealth() * amount/100;
                float minval = m_target->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_HEALING_PCT);
                if(minval)
                    tempDamage *= (100.0f + minval) / 100.0f;

                pdamage = uint32(tempDamage);
            }
            else
            ...
            if(m_target!=pCaster && spellProto->SpellVisual==163)
            {
                ...
                {
                    pCaster->SendSpellNonMeleeDamageLog(pCaster, GetId(), dmg, GetSpellSchoolMask(GetSpellProto()), 0, 0, false, 0, false);

                    CleanDamage cleanDamage =  CleanDamage(0, BASE_ATTACK, MELEE_HIT_NORMAL );
                    pCaster->DealDamage(pCaster, dmg, &cleanDamage, NODAMAGE, GetSpellSchoolMask(GetSpellProto()), GetSpellProto(), true);
                }
            }
