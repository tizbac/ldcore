...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    if (m_healing > 0)
    {
        ...
        if (missInfo != SPELL_MISS_REFLECT)
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, addhealth, 0, m_attackType, m_spellInfo, m_canTrigger);
    ...
    else if (m_damage > 0)
    {
        ...
        if (missInfo != SPELL_MISS_REFLECT)
        {
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, damageInfo.damage, damageInfo.cleanDamage, m_attackType, m_spellInfo, m_canTrigger);
            if(caster->GetTypeId() == TYPEID_PLAYER)
    ...
    else
    {
        ...
        // Do triggers for unit (reflect triggers passed on hit phase for correct drop charge)
        if (missInfo != SPELL_MISS_REFLECT)
            caster->ProcDamageAndSpell(unit, procAttacker, procVictim, procEx, 0, 0, m_attackType, m_spellInfo, m_canTrigger);
    }
...
void Spell::DoSpellHitOnUnit(Unit *unit, const uint32 effectMask)
{
    ...
    if( m_caster != unit )
    {
        ...
        if( !m_caster->IsFriendlyTo(unit) )
        {
            ...
            bool isVisibleForHit = ( (unit->HasAuraType(SPELL_AURA_MOD_INVISIBILITY) || unit->HasAuraTypeWithFamilyFlags(SPELL_AURA_MOD_STEALTH, SPELLFAMILY_ROGUE ,SPELLFAMILYFLAG_ROGUE_VANISH)) && (!m_caster->canSeeOrDetect(unit, true) || !m_caster->canDetectStealthOf(unit, m_caster->GetDistance(unit))) ) ? false : true;
    ...
    while(num)
    {
        ...
        else
        {
            ...
            while(m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MELEE
                ...
                || !m_caster->canDetectStealthOf(*next, m_caster->GetDistance(*next))
                || !cur->IsWithinLOSInMap(*next))
            {
                ++next;
...
void Spell::SetTargetMap(uint32 i, uint32 cur)
{
    ...
    switch(spellmgr.SpellTargetType[cur])
    {
        ...
        case TARGET_TYPE_DEST_TARGET: //2+8+2
        {
            ...
            for(float moveStep = dist; moveStep > 0; moveStep-=0.5f)
            {
                target->GetGroundPointAroundUnit(x, y, z, moveStep, angle);
                float fFloor  = m_caster->GetMap()->GetHeight(x, y, z, true);
                float fGround = m_caster->GetMap()->GetHeight(x, y, MAX_HEIGHT);

                z = fFloor > fGround ? fFloor : fGround;

                if(fabs(fCentZ-z) < 2.0f)
                    break;
            }
    ...
    if(m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if(hit && NeedsComboPoints(m_spellInfo))
        {
            ...
            if(((Player*)m_caster)->GetComboPointsDelayed())
                cptarget = ObjectAccessor::GetUnit(*m_caster,((Player*)m_caster)->GetComboTarget());
...
{
    if  (m_caster->GetTypeId() == TYPEID_PLAYER && (m_spellInfo->Attributes & SPELL_ATTR_DISABLED_WHILE_ACTIVE) &&
        (m_caster->HasAura(m_spellInfo->Id, 0) || m_caster->HasAura(m_spellInfo->Id, 1) || m_caster->HasAura(m_spellInfo->Id, 2)))
...
uint8 Spell::CheckItems()
{
    ...
    if(m_spellInfo->RequiresSpellFocus)
    {
        ...
        cell.Visit(p, object_checker, *MapManager::Instance().GetMap(m_caster->GetMapId(), m_caster));
