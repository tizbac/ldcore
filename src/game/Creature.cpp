...
bool Creature::canDetectStealthOf(Unit const* target, float distance) const
{
    ...
    if( !m_AlreadyCallAssistance && getVictim() && !isPet() && !isCharmed())
    {
        ...
        if(radius > 0)
        {
...

            {
                ...
                cell.Visit(p, grid_creature_searcher, *MapManager::Instance().GetMap(GetMapId(), this));
            }
