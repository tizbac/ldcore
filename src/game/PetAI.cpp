...
void PetAI::UpdateAI(const uint32 diff)
{
    ...
    {
        if(m_creature->getVictim()->HasAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_DAMAGE))
        {
            if(!m_creature->IsForceAttack())
            {
                _stopAttack();
        ...
        {
            m_creature->SetForceAttack(false);
        }
    ...
    else if(owner && m_creature->GetCharmInfo()) //no victim
    {
        ...
        if (nextTarget && nextTarget->HasAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_DAMAGE))
        {
            if(!m_creature->IsForceAttack())
            {
                _stopAttack();
...
void PetAI::MovementInform(uint32 moveType, uint32 data)
{
    ...
    switch (moveType)
    {
        case POINT_MOTION_TYPE:
        {
            ...
            if (me->GetCharmInfo() && data == me->GetGUIDLow() && me->GetCharmInfo()->IsReturning())
            {
                me->GetCharmInfo()->SetIsAtStay(true);
                me->GetCharmInfo()->SetIsReturning(false);
                me->GetCharmInfo()->SetIsFollowing(false);
