...
void BattleGround::EndBattleGround(uint32 winner)
{
    ...
    m_EndTime = 0;
    bool assignPoint = true;

    uint32 totalDamageHorde = 0;
    uint32 totalHealHorde   = 0;
    uint32 totalPlayerHorde = 0;

    uint32 totalDamageAlly  = 0;
    uint32 totalHealAlly    = 0;
    uint32 totalPlayerAlly  = 0;
    ...
    if(isArena() && isRated())
    {
        for(std::map<uint64, BattleGroundPlayer>::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        {
            ...
            std::map<uint64, BattleGroundScore*>::iterator plrScore = m_PlayerScores.find(plr->GetGUID());
            if(plrScore != m_PlayerScores.end())
            {
                if(team == ALLIANCE)
                {
                    totalPlayerAlly++;
                    totalDamageAlly += plrScore->second->DamageDone;
                    totalHealAlly   += plrScore->second->HealingDone;
                }

                if(team == HORDE)
                {
                    totalPlayerHorde++;
                    totalDamageHorde += plrScore->second->DamageDone;
                    totalHealHorde   += plrScore->second->HealingDone;
                }
            }
            ...
            if(accId)
            {
                ...
                if(result)
                {
                    ...
                    for(std::map<uint64, BattleGroundPlayer>::iterator itr_ = m_Players.begin(); itr_ != m_Players.end(); ++itr_)
                    {
                        Player *plr_ = objmgr.GetPlayer(itr_->first);
                        if(!plr_)
                        {
                            continue;
                        }

                        uint32 team_ = itr_->second.Team;

                        if(!team_)
                            team_ = plr_->GetTeam();

                        if(team_ == team || plrGUID == plr_->GetGUID())
                            continue;

                        uint32 accId_ = 0;
                        accId_ = plr_->GetSession()->GetAccountId();
                        std::string last_ip_ = GetTrinityString(LANG_ERROR);
                        QueryResult* result_ = LoginDatabase.PQuery("SELECT last_ip FROM account WHERE id = '%u'",accId_);
                        if(result_)
                        {
                            Field* fields_ = result_->Fetch();
                            last_ip_ = fields_[0].GetCppString();

                            if(last_ip.compare(last_ip_) == 0)
                            {
                                assignPoint = false;
                            }
    ...
    if(isArena() && isRated() && assignPoint)
    {
        if(winner == ALLIANCE)
        {
        ...
        if(winner_arena_team && loser_arena_team)
        {
            ...
            CharacterDatabase.PExecute("INSERT INTO arena_log (action,a_teamid,a_dmg,a_heal,a_plrcount,b_teamid,b_dmg,b_heal,b_plrcount,winner,rating_change) "
                "VALUES('%u','%u','%u','%u','%u','%u','%u','%u','%u','%u','%u')", 6, m_ArenaTeamIds[BG_TEAM_ALLIANCE], totalDamageAlly, totalHealAlly, totalPlayerAlly, m_ArenaTeamIds[BG_TEAM_HORDE], totalDamageHorde, totalHealHorde, totalPlayerHorde, winner_arena_team->GetId(), winner_change);
            CharacterDatabase.CommitTransaction();
            //sLog.outArena("END Arena match [Type: %u] [Team1Id: %u | dmg: %u | heal: %u | plr %u] vs [Team2Id: %u | dmg: %u | heal: %u | plr %u] [WinnerTeamId: %u][RatingChange: %i]", m_ArenaType, m_ArenaTeamIds[BG_TEAM_ALLIANCE], totalDamageAlly, totalHealAlly, totalPlayerAlly, m_ArenaTeamIds[BG_TEAM_HORDE], totalDamageHorde, totalHealHorde, totalPlayerHorde, winner_arena_team->GetId(), winner_change);
        }
        else
        {
    ...
    for(std::map<uint64, BattleGroundPlayer>::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if(isArena() && isRated() && assignPoint && winner_arena_team && loser_arena_team)
        {
            if(team == winner)
    ...
    if(plr)
    {
        ...
        if(participant) // if the player was a match participant, remove auras, calc rating, update queue
        {
            ...
            if(isArena())
            {
                ...
                {
                    CharacterDatabase.BeginTransaction();
                    CharacterDatabase.PExecute("INSERT INTO arena_log (action,a_teamid,b_teamid,player_source) "
                        "VALUES('%u','%u','%u','%u')", 7, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE], plr->GetGUIDLow());
                    CharacterDatabase.CommitTransaction();
//                    sLog.outArena("Player: %s [GUID: %u] left match type: %u - Teamd1id: %u - Team2id: %u", plr->GetName(), plr->GetGUIDLow(), GetArenaType(), m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE]);
                }
...
void BattleGround::StartBattleGround()
{
    ...
    {
        CharacterDatabase.BeginTransaction();
        CharacterDatabase.PExecute("INSERT INTO arena_log (action,a_teamid,b_teamid) "
            "VALUES('%u','%u','%u')", 8, m_ArenaTeamIds[BG_TEAM_ALLIANCE], m_ArenaTeamIds[BG_TEAM_HORDE]);
        CharacterDatabase.CommitTransaction();
...
uint32 BattleGround::GetFreeSlotsForTeam(uint32 Team, bool isGroup) const
{
